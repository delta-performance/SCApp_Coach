...
// _firebase-entry.js
var _authInstances = /* @__PURE__ */ new Map();
function getAuth2(app) {
  const key = app ? app.name : "[DEFAULT]";
  if (_authInstances.has(key)) return _authInstances.get(key);
  const auth = getAuth(app);
  setPersistence(auth, browserLocalPersistence).catch(() => {
  });
  _authInstances.set(key, auth);
  return auth;
}
function setDoc2(...args) {
  if (typeof window !== "undefined" && window.setDoc && (!args[0] || !args[0].__inWriteWrapper)) {
    return window.setDoc(...args);
  }
  return setDoc(...args);
}
function addDoc2(...args) {
  if (typeof window !== "undefined" && window.addDoc && (!args[0] || !args[0].__inWriteWrapper)) {
    return window.addDoc(...args);
  }
  return addDoc(...args);
}
function updateDoc2(...args) {
  if (typeof window !== "undefined" && window.updateDoc && (!args[0] || !args[0].__inWriteWrapper)) {
    return window.updateDoc(...args);
  }
  return updateDoc(...args);
}
function deleteDoc2(...args) {
  if (typeof window !== "undefined" && window.deleteDoc && (!args[0] || !args[0].__inWriteWrapper)) {
    return window.deleteDoc(...args);
  }
...
function getFirestore2(app) {
  const key = app ? app.name : "[DEFAULT]";
  if (_firestoreInstances.has(key)) return _firestoreInstances.get(key);
  let db;
  try {
  ...
  _firestoreInstances.set(key, db);
  return db;
}
export {
  addDoc2 as addDoc,
