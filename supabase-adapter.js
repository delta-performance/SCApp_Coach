...
var PostgrestFilterBuilder = class extends PostgrestTransformBuilder {
  ...
  */
  ...
  textSearch(column, query2, { config, type } = {}) {
    let typePart = "";
...
// _supabase-entry.js
var SUPABASE_URL = "https://ywtrsuoqbtjlsumwdmjw.supabase.co";
var SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inl3dHJzdW9xYnRqbHN1bXdkbWp3Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODUyNzA5MTIsImV4cCI6MjEwMDg0NjkxMn0.SuW_0ak01r4MnHzk_ynTaJy_go8WxrxDvJRRtWZWprE";
var DEFAULT_CLUB_ID = "00000000-0000-0000-0000-000000000001";
var supabase = null;
var _auth = null;
var COLLECTION_TO_TABLE = {
  "joueurs": "joueurs",
  "seances": "seances",
  "cycles": "cycles",
  "cyclesEnergetique": "cycles_energetique",
  "seancesEnergetique": "seances_energetique",
  "retoursEnergetique": "retours_energetique",
  "fiches_seances": "fiches_seances",
  "bip_sessions": "bip_sessions",
  "groupesConfiguration": "groupes_configuration",
  "blocsBonus": "blocs_bonus",
  "weeksConfig": "weeks_config",
  "entrainements": "entrainements",
  "dataPerf": "data_perf",
  "maxTestes": "max_testes",
  "chargesReelles": "charges_reelles",
  "rfuData": "rfu_data",
  "wellness": "wellness",
  "poids": "poids",
  "statsGPS": "stats_gps",
  "planificationsGPS": "planifications_gps",
  "dispoKine": "dispo_kine",
  "blessures": "blessures",
  "absences": "absences",
  "bilans": "bilans",
  "objectifsBlesses": "objectifs_blesses",
  "creneauxBlesses": "creneaux_blesses",
  "planning": "planning",
  "matchs_calendrier": "matchs_calendrier",
  "matchs_phases": "matchs_phases",
  "feuillesMatch": "feuilles_match",
  "ffr_match_data": "ffr_match_data",
  "vitesseData": "vitesse_data",
  "puissanceData": "puissance_data",
  "fv_profils": "fv_profils",
  "gps": "stats_gps",
...
var DATA_JSONB_TABLES = /* @__PURE__ */ new Set(["blessures", "absences", "joueurs", "creneaux_blesses", "planifications_blesses", "stats_gps", "fv_profils", "vitesse_data", "puissance_data", "progressions_bonus", "change_log", "morpho_overrides", "entrainements", "feuilles_match"]);
...
var TABLE_COLUMNS = {
  ...
  creneaux_blesses: /* @__PURE__ */ new Set([
    "id",
    "club_id",
    "date",
    "heure_debut",
...
function convertKeysToSnake(obj) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    result[snakeKey] = isPlainObject2(value) ? convertKeysToSnake(value) : value;
  }
  return result;
}
function convertKeysToCamel(obj) {
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    result[camelKey] = isPlainObject2(value) ? convertKeysToCamel(value) : value;
  }
  return result;
}
...
function getAuth(app) {
  ...
  supabase.auth.getSession().then(({ data }) => {
    ...
    _auth._listeners.forEach((cb) => cb(_auth.currentUser));
  });
  supabase.auth.onAuthStateChange((event, session) => {
    if (session) {
      _auth.currentUser = {
...
function onAuthStateChanged(auth, callback) {
  ...
  if (auth._sessionReady && auth.currentUser !== void 0) {
    callback(auth.currentUser);
  }
  return () => {
    const idx = auth._listeners.indexOf(callback);
    if (idx >= 0) auth._listeners.splice(idx, 1);
  };
}
...
async function createUserWithEmailAndPassword(auth, email, password) {
  ...
  if (error) throw { code: "auth/email-already-in-use", message: error.message };
  return { user: data.user };
}
...
async function sendPasswordResetEmail(auth, email) {
  const redirectUrl = typeof window !== "undefined" && window.__RESET_PASSWORD_URL__ || "https://delta-performance.github.io/delta-reset-password/";
  const { error } = await auth._supabase.auth.resetPasswordForEmail(email, {
...
function where(field, op, value) {
  return { __type: "where", __field: field, __op: op, __value: value };
}
function rowMatchesConstraints(row, constraints) {
  if (!constraints || !constraints.length) return true;
  for (const c of constraints) {
    if (!c || c.__type !== "where") continue;
    const field = camelToSnake(c.__field);
    const op = c.__op;
    const val = c.__value;
    let queryVal = val;
    if (val instanceof Date) queryVal = val.toISOString();
    const rowVal = row ? row[field] : void 0;
    if (op === "==" || op === "=") {
...
function orderBy(field, dir) {
  return { __type: "orderBy", __field: field, __dir: dir || "asc" };
}
function query(ref, ...constraints) {
  return {
    __type: "query",
    __collectionName: ref.__collectionName,
    __table: ref.__table || COLLECTION_TO_TABLE[ref.__collectionName] || ref.__collectionName,
    __constraints: [...ref.__constraints || [], ...constraints],
    __db: ref.__db
  };
}
function limit(n) {
  return { __type: "limit", __value: n };
}
function startAfter(cursor) {
  return { __type: "startAfter", __value: cursor };
}
function applyConstraints(supabaseQuery, constraints) {
  for (const c of constraints) {
    if (!c) continue;
    if (c.__type === "where") {
      const field = camelToSnake(c.__field);
      ...
      if (val instanceof Date) {
        queryVal = val.toISOString();
      }
      if (op === "==" || op === "=") supabaseQuery = supabaseQuery.eq(field, queryVal);
      else if (op === "!=") supabaseQuery = supabaseQuery.neq(field, queryVal);
      else if (op === ">") supabaseQuery = supabaseQuery.gt(field, queryVal);
      else if (op === ">=") supabaseQuery = supabaseQuery.gte(field, queryVal);
      else if (op === "<") supabaseQuery = supabaseQuery.lt(field, queryVal);
      else if (op === "<=") supabaseQuery = supabaseQuery.lte(field, queryVal);
      else if (op === "in") supabaseQuery = supabaseQuery.in(field, queryVal);
      else if (op === "array-contains") supabaseQuery = supabaseQuery.contains(field, [queryVal]);
      else if (op === "array-contains-any") supabaseQuery = supabaseQuery.contains(field, queryVal);
      else if (op === "!=") supabaseQuery = supabaseQuery.neq(field, queryVal);
    } else if (c.__type === "orderBy") {
      const field = camelToSnake(c.__field);
      supabaseQuery = supabaseQuery.order(field, { ascending: c.__dir === "asc" });
    } else if (c.__type === "limit") {
      supabaseQuery = supabaseQuery.limit(c.__value);
    }
  }
...
async function getMaxTestesDoc(supabase2, docRef) {
  const { data, error } = await supabase2.from("max_testes").select("joueur_id, exercice, valeur").eq("joueur_id", docRef.__id);
  if (error) throw error;
  if (!data || data.length === 0) return wrapEmpty(docRef.__id);
  const result = { id: docRef.__id };
  for (const row of data) {
    if (row.exercice) result[row.exercice] = row.valeur;
  }
  return {
    id: docRef.__id,
    exists: () => true,
...
async function setMaxTestesDoc(supabase2, docRef, dataObj, options) {
  const joueurId = docRef.__id;
  ...
  for (const [key, value] of Object.entries(dataObj)) {
    ...
    if (value && value.__deleteField) {
      ...
      upsertRows.push({
        ...
        joueur_id: joueurId,
        exercice: key,
        valeur: typeof value === "number" ? value : parseFloat(value),
        updated_at: now
      });
    }
  }
  ...
  if (upsertRows.length > 0) {
    ops.push(supabase2.from("max_testes").upsert(upsertRows, { onConflict: "joueur_id,exercice" }));
  }
  for (const key of deleteKeys) {
    ops.push(supabase2.from("max_testes").delete().eq("joueur_id", joueurId).eq("exercice", key));
  }
  ...
  for (const { error } of results) {
    if (error) {
      batchFailed = true;
      console.warn("[setMaxTestesDoc] Batch error:", error.message);
    }
  }
  if (batchFailed && upsertRows.length > 0) {
    console.warn("[setMaxTestesDoc] Batch failed, retrying row by row");
    const singleOps = upsertRows.map(
      (row) => supabase2.from("max_testes").upsert(row, { onConflict: "joueur_id,exercice" })
    );
    for (const key of deleteKeys) {
      singleOps.push(supabase2.from("max_testes").delete().eq("joueur_id", joueurId).eq("exercice", key));
    }
    const retryResults = await Promise.all(singleOps);
    for (const { error } of retryResults) {
      if (error) throw error;
    }
  }
}
async function getMaxTestesDocs(supabase2, queryRef) {
  let q = supabase2.from("max_testes").select("joueur_id, exercice, valeur");
  q = applyConstraints(q, queryRef.__constraints || []);
  const { data, error } = await q;
  if (error) throw error;
  const grouped = {};
  for (const row of data) {
    if (!grouped[row.joueur_id]) grouped[row.joueur_id] = { id: row.joueur_id };
    if (row.exercice) grouped[row.joueur_id][row.exercice] = row.valeur;
  }
  return Object.values(grouped).map((g) => wrapDoc(g, g.id));
}
...
async function setChargesReellesDoc(supabase2, docRef, dataObj, options) {
  ...
  if (batchFailed && upsertRows.length > 0) {
    console.warn("[setChargesReellesDoc] Batch failed, retrying row by row");
    const singleOps = upsertRows.map(
      (row) => supabase2.from("charges_reelles").upsert(row, { onConflict: "joueur_id,seance_id,bloc_id,groupe_id,exercice_id,numero_serie" })
    );
    singleOps.push(...deleteOps);
    const retryResults = await Promise.all(singleOps);
    for (const { error } of retryResults) {
      if (error) throw error;
    }
...
async function deleteChargesReellesDoc(supabase2, docRef) {
  ...
  if (seanceId && joueurId) {
    await supabase2.from("charges_reelles").delete().eq("seance_id", seanceId).eq("joueur_id", joueurId);
  }
}
async function getConfigDoc(supabase2, docRef) {
  const docId = docRef.__id;
  const { data, error } = await supabase2.from("config_generic").select("data").eq("id", docId).limit(1);
  if (error || !data || data.length === 0) return wrapEmpty(docId);
...
async function setConfigDoc(supabase2, docRef, dataObj, options) {
  ...
  if (isMerge) {
    const existing = await getConfigDoc(supabase2, docRef);
    if (existing.exists()) {
      finalData = { ...existing.data(), ...dataObj };
    }
  }
  await supabase2.from("config_generic").upsert({ id: docId, data: finalData, club_id: DEFAULT_CLUB_ID, updated_at: (/* @__PURE__ */ new Date()).toISOString() });
}
async function getCompositeKeyDoc(supabase2, docRef) {
  const id = docRef.__id;
  const lastUnderscore = id.lastIndexOf("_");
  if (lastUnderscore === -1) return wrapEmpty(id);
  const joueurId = id.substring(0, lastUnderscore);
  const date = id.substring(lastUnderscore + 1);
  const { data, error } = await supabase2.from(docRef.__table).select("*").eq("joueur_id", joueurId).eq("date", date).maybeSingle();
  if (error || !data) return wrapEmpty(id);
  return wrapDoc(data, data.id || id);
}
async function getDoc(docRef) {
  const supabase2 = docRef.__db;
  const table = docRef.__table;
  const id = docRef.__id;
  if (!supabase2 || !table) {
    return wrapEmpty(id);
  }
  if (table === "max_testes") return getMaxTestesDoc(supabase2, docRef);
  if (table === "charges_reelles") return getChargesReellesDocV2(supabase2, docRef);
  if (docRef.__collectionName === "config") return getConfigDoc(supabase2, docRef);
  if (table === "wellness" || table === "poids") return getCompositeKeyDoc(supabase2, docRef);
  const { data, error } = await supabase2.from(table).select("*").eq("id", id).single();
  if (error || !data) return wrapEmpty(id);
  return wrapDoc(data, id);
}
var TABLE_SELECT_COLUMNS = {
  // stats_gps: top3_global and joueurs JSONB are huge but needed for classement
...
async function getDocsRaw(queryRef) {
  const supabase2 = queryRef.__db;
...
async function getDocs(queryRef) {
  if (typeof window !== "undefined" && window.getDocs) {
    return window.getDocs(queryRef);
  }
  return getDocsRaw(queryRef);
}
async function setDoc(docRef, dataObj, options) {
  if (typeof window !== "undefined" && window.setDoc && (!docRef || !docRef.__inWriteWrapper)) {
    return window.setDoc(docRef, dataObj, options);
  }
  const supabase2 = docRef.__db;
  const table = docRef.__table;
  const id = docRef.__id;
  const isMerge = options && options.merge;
  if (table === "max_testes") return setMaxTestesDoc(supabase2, docRef, dataObj, options);
  if (table === "charges_reelles") return setChargesReellesDoc(supabase2, docRef, dataObj, options);
  if (docRef.__collectionName === "config") return setConfigDoc(supabase2, docRef, dataObj, options);
  ...
  if (DATA_JSONB_TABLES.has(table) && TABLE_COLUMNS[table]) {
    ...
    if (Object.keys(extraFields).length > 0) {
      if (isMerge) {
        const { data: existing } = await supabase2.from(table).select("data").eq("id", id).single();
        const existingData = existing && existing.data || {};
        rowData.data = { ...existingData, ...extraFields };
      } else {
        rowData.data = extraFields;
      }
    }
  }
  for (const [key, value] of Object.entries(rowData)) {
  ...
  if (uuidConflict !== void 0) {
    if (uuidConflict) {
      ...
      const { error } = await supabase2.from(table).insert(rowData);
      if (error) throw error;
    }
  } else if (isMerge) {
...
async function addDoc(collectionRef, dataObj) {
  if (typeof window !== "undefined" && window.addDoc && (!collectionRef || !collectionRef.__inWriteWrapper)) {
    return window.addDoc(collectionRef, dataObj);
  }
  const supabase2 = collectionRef.__db;
  const table = collectionRef.__table;
  let rowData = convertKeysToSnake(dataObj);
  rowData.club_id = DEFAULT_CLUB_ID;
  if (!rowData.id) rowData.id = crypto.randomUUID();
  if (DATA_JSONB_TABLES.has(table) && TABLE_COLUMNS[table]) {
    const knownCols = TABLE_COLUMNS[table];
    const extraFields = {};
    for (const [key, value] of Object.entries(rowData)) {
  ...
  const { data, error } = await supabase2.from(table).insert(rowData).select();
  if (error) throw error;
  return { id: data[0]?.id || crypto.randomUUID() };
}
async function updateDoc(docRef, dataObj) {
  if (typeof window !== "undefined" && window.updateDoc && (!docRef || !docRef.__inWriteWrapper)) {
    return window.updateDoc(docRef, dataObj);
  }
  const supabase2 = docRef.__db;
  const table = docRef.__table;
  const id = docRef.__id;
  ...
  if (DATA_JSONB_TABLES.has(table) && TABLE_COLUMNS[table]) {
    ...
    if (Object.keys(extraFields).length > 0) {
      const { data: existing } = await supabase2.from(table).select("data").eq("id", id).single();
      const existingData = existing && existing.data || {};
      updateData.data = { ...existingData, ...extraFields };
    }
  }
  for (const [key, value] of Object.entries(updateData)) {
    if (value && value.__deleteField) {
...
async function deleteDoc(docRef) {
  if (typeof window !== "undefined" && window.deleteDoc && (!docRef || !docRef.__inWriteWrapper)) {
...
var _sharedDocChannels = {};
function getOrCreateSharedDocChannel(supabase2, table) {
  if (!_sharedDocChannels[table]) {
    const listeners = /* @__PURE__ */ new Map();
    const channel = supabase2.channel("shared_doc_" + table);
    channel.on(
      "postgres_changes",
      { event: "*", schema: "public", table },
      (payload) => {
        const rowId = payload.new && payload.new.id || payload.old && payload.old.id || payload.record && payload.record.id;
        if (!rowId) return;
        const cbs = listeners.get(rowId);
        if (cbs) {
          cbs.forEach((cb) => {
            try {
              supabase2.from(table).select("*").eq("id", rowId).limit(1).then(({ data, error }) => {
                if (error || !data || data.length === 0) {
                  cb(wrapEmpty(rowId));
                } else {
                  const rowData = data[0];
                  cb(wrapDoc(rowData, rowId));
                }
              }).catch((e) => console.error("shared doc channel refetch error:", e));
            } catch (e) {
              console.error("shared doc channel callback error:", e);
            }
          });
        }
      }
    ...
    channel.subscribe();
    _sharedDocChannels[table] = { channel, listeners };
  }
  return _sharedDocChannels[table];
}
function onSnapshot(ref, ...args) {
  const supabase2 = ref.__db;
  const table = ref.__table;
  const isDoc = ref.__type === "doc";
  let callback;
  if (typeof args[0] === "function") {
    callback = args[0];
  } else if (args[0] && typeof args[0].next === "function") {
    callback = args[0].next;
  } else if (args[0] && typeof args[0] === "object") {
    console.warn("onSnapshot: observer object without next callback", args[0]);
    return () => {
    };
  } else {
    console.warn("onSnapshot: no valid callback in args", args);
    return () => {
    };
  }
  if (!supabase2 || !table) {
    console.warn("onSnapshot: no supabase/table for ref", ref);
    return () => {
    };
  }
  const queryConstraints = ref.__constraints || [];
  let cachedRows = null;
  let cachedDocs = null;
  let pendingRefetch = null;
  let pendingEmit = null;
  function wrapSingleDoc(row) {
    return wrapDoc(row);
  }
  function rebuildDocsFromRows(rows) {
    cachedDocs = rows.map((r) => wrapSingleDoc(r));
  }
  ...
  const fetchInitial = async () => {
    if (isDoc) {
      ...
      if (snap && snap.docs) {
        cachedRows = snap.docs.map((d) => {
          const data = d.data();
          return convertKeysToSnake({ id: d.id, ...data });
        });
        rebuildDocsFromRows(cachedRows);
      }
      callback(snap);
    }
  };
  fetchInitial().catch((e) => console.error("onSnapshot initial fetch error:", e));
  if (isDoc) {
    const shared = getOrCreateSharedDocChannel(supabase2, table);
    const docId = ref.__id;
    if (!shared.listeners.has(docId)) {
      shared.listeners.set(docId, /* @__PURE__ */ new Set());
    }
    shared.listeners.get(docId).add(callback);
    return () => {
      const cbs = shared.listeners.get(docId);
      if (cbs) {
        cbs.delete(callback);
        if (cbs.size === 0) {
          shared.listeners.delete(docId);
        }
      }
      if (shared.listeners.size === 0) {
        supabase2.removeChannel(shared.channel);
        delete _sharedDocChannels[table];
      }
    };
  }
    ...
    (payload) => {
      if (!cachedRows || !Array.isArray(cachedRows)) {
        getDocs(ref).then((snap) => {
          if (snap && snap.docs) {
            cachedRows = snap.docs.map((d) => convertKeysToSnake({ id: d.id, ...d.data() }));
            rebuildDocsFromRows(cachedRows);
          }
          callback(snap);
      ...
      if (eventType === "INSERT" && payload.new) {
        ...
        if (!exists) {
          ...
          cachedDocs = [...cachedDocs || [], wrapSingleDoc(payload.new)];
        }
        ...
        if (wasInCache && matchesNow) {
          ...
          cachedRows = cachedRows.filter((r) => r.id !== payload.new.id);
          cachedDocs = (cachedDocs || []).filter((d) => d.id !== payload.new.id);
        } else if (!wasInCache && matchesNow) {
          cachedRows = [...cachedRows, payload.new];
          cachedDocs = [...cachedDocs || [], wrapSingleDoc(payload.new)];
        }
      } else if (eventType === "DELETE" && payload.old) {
        cachedRows = cachedRows.filter((r) => r.id !== payload.old.id);
        cachedDocs = (cachedDocs || []).filter((d) => d.id !== payload.old.id);
      } else {
        if (!pendingRefetch) {
  ...
  return {
    ...
    async commit() {
      const promises = operations.map((op) => {
        if (op.type === "set") return setDoc(op.docRef, op.data);
...
var GPS_BUCKET = "gps-data";
async function uploadGPSFile(file, fileName) {
  if (!supabase) {
    supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
      auth: { persistSession: true, autoRefreshToken: true }
    });
  }
  const { data, error } = await supabase.storage.from(GPS_BUCKET).upload(fileName || file.name, file, {
    upsert: true,
...
async function downloadGPSFile(fileName) {
  if (!supabase) {
    supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
      auth: { persistSession: true, autoRefreshToken: true }
    });
  }
  const { data, error } = await supabase.storage.from(GPS_BUCKET).download(fileName);
  if (error) throw error;
  return data;
}
async function listGPSFiles() {
  if (!supabase) {
    supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
      auth: { persistSession: true, autoRefreshToken: true }
    });
  }
  const { data, error } = await supabase.storage.from(GPS_BUCKET).list();
  if (error) throw error;
  return data || [];
}
async function getGPSFileInfo(fileName) {
  if (!supabase) {
    supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
      auth: { persistSession: true, autoRefreshToken: true }
    });
  }
  const { data, error } = await supabase.storage.from(GPS_BUCKET).list("", { search: fileName });
  if (error) throw error;
  return data && data[0] || null;
}
var memoryLocalCache = () => ({});
function initializeFirestore(app, options) {
  return getFirestore(app);
}
var getDocsOriginal = getDocsRaw;
export {
  GoogleAuthProvider,
  addDoc,
